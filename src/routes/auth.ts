...
route.post("/login", async (req: any, res: any, next) => {
    ...
    try {
        ...
        respond(res, req, 200, "Login successful", { jwt: jwttoken, picture: user_google_object.picture, username: user.name, email: user.email });
    }
...
route.get("/user", async (req: any, res: any, next) => {
    ...
    if (typeof bearer_header !== undefined) {
        ...
        try {
            ...
            user ? respond(res, req, 200, "Fetched user details successfully", { username: user.name, email: user.email, picture: user.picture })
